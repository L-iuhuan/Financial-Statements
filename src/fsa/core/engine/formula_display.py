...
仅用于显示层 (GUI 结果卡片 / 审计底稿导出 / Agent 诊断), 不影响引擎求值
(引擎仍用英文变量)。
示例: "asset_total == liability_total + equity_total"
   -> "资产总计 == 负债合计 + 所有者权益合计"

2026-09-18 从 gui/formula_display.py 移入 core: 审计底稿导出器
(core/exporter) 需要中文公式列, 而 core 禁止 import gui; gui 侧保留
兼容再导出 (gui/formula_display.py)。
"""
...
def _translate_token(token: str) -> str:
    ...
    # 行业阈值变量 (LR-* 公式, 如 dar_threshold/current_ratio_threshold):
    # 具体数值按行业可配置, 统一译为「行业阈值」
    if token.endswith("_threshold"):
        return "行业阈值"

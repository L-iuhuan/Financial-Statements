...
| 文件 | 职责 |
|---|---|
| `database.py` | `Database`：默认 `~/.fsa/data.db`；`init_schema` 幂等 DDL（validation_history / validation_results / chat_sessions / chat_messages / rule_overrides）；支持 `with Database() as db` |
...
| `override_repo.py` | 规则容差/启停覆写 UPSERT（`ON CONFLICT(rule_id) DO UPDATE`） |
...
- DDL 必须幂等（`CREATE TABLE IF NOT EXISTS`），`init_schema` 可重复执行

...
## 2026-08-15 | v0.4.1 体验修复

### 完成事项

- **历史查看**: 改为跳转「校验结果」表格页展示，导入页不再重建结果卡片；实测查看历史耗时约 0.02s，消除窗口闪烁。
- **tab 命名**: 侧边栏/顶栏「审计底稿」更名为「校验结果」；导出按钮保留「导出 Excel 底稿」。
- **首页文件入口**: 移除“等待导入财务报表”重复虚线空状态，保留唯一文件区并支持点击选择文件（多选 xlsx/xls/pdf）。
- **主题切换**: 隐藏抽屉不再预载全部历史消息；隐藏消息气泡/结果卡片主题刷新延迟到显示时，主题切换耗时显著下降。
- **版本**: 升级至 `0.4.1`。

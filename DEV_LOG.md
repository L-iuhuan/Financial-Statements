...
- [ ] A1. 重新设计 Demo HTML - 去除"AI 味"，校验概览/明细卡片改为更克制的财务风格
- [ ] A2. 实现自定义侧边栏（logo + 分区标签 + 底部版本信息）
- [ ] A3. 实现顶栏组件（页面标题 + 副标题 + 操作按钮组：主题切换/重置/执行校验/导出底稿）
- [ ] A4. 实现汇总卡片组件（4 列：通过/错误/警告/总数，克制设计）
- [ ] A5. 实现筛选标签栏（全部/错误/警告/通过，带计数）
- [ ] A6. 重建校验明细卡片（可展开、含追溯表、AI 诊断按钮）
- [ ] A7. 新增审计底稿页面（表格形式展示全部校验结果）
- [ ] A8. 新增历史记录页面
- [ ] A9. 完善系统设置页面（主题选项/容差参数/数据库路径/历史保留天数/关于）
- [ ] A10. 主题系统重建 - 正确实现明暗双套配色

### B. AI 助手抽屉 (优先级: 高)

- [ ] B1. 实现可拖拽缩放的抽屉面板（QSplitter 或自定义拖拽手柄）
- [ ] B2. 点击抽屉外区域自动收起，再点击浮动按钮打开
- [ ] B3. 问答内容持久化（SQLite 表存储对话记录）
- [ ] B4. 跨页面保持问答上下文（全局单例面板）
- [ ] B5. 内网同步：问答内容导出/同步到更新文件夹
- [ ] B6. 每张校验卡片独立 AI 诊断按钮 -> 打开抽屉并预填上下文

### C. 核心功能补全 (优先级: 中)

- [ ] C1. SQLite 持久化层（存储报表、校验结果、校验历史）
- [ ] C2. Excel 导出器（审计底稿导出，含公式追溯）
- [ ] C3. LR-* 规则公式扩展（支持范围/条件表达式）
- [ ] C4. 测试数据丰富化（真实财务场景数据，覆盖更多科目变量）

### D. 规划后续功能 (优先级: 低)

- [ ] D1. PDF 导入 (V1)
- [ ] D2. 报表自动生成 (V1.5)
- [ ] D3. Ollama AI 诊断引擎接入 (V1)
- [ ] D4. 自动更新模块 (V1.0)
- [ ] D5. Inno Setup 安装器 (MVP 收尾)
- [ ] D6. 间接法现金流量表 (V2.0)
- [ ] D7. 合并报表 (V2.0)
- [ ] D8. IFRS 支持 (V2.0)
